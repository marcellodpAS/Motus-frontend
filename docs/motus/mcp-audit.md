...
✅ **Verifica aggiuntiva**: `ListMcpResourcesTool` (senza filtro `server`, quindi su tutti i server MCP connessi) → risposta: _"No resources found. MCP servers may still provide tools even if they have no resources."_ Nessuna risorsa MCP di alcun tipo, design o altro, è esposta in questa sessione.
...
> _"Temporary technical tokens. Replace these values after the Stitch design analysis"_
